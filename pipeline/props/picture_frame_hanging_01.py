# Hanging oak frame 50x70 with ivory mount, dusk-hills print. Back at -Z (wall).
import sys; sys.path.insert(0, __import__('os').path.dirname(__import__('os').path.abspath(__file__)))
import helpers as H, _kit as K, _photo
from helpers import V, PI
import math
H.reset()
frame = K.wood('hang_oak', seed=140)
mount = K.plain('hang_mount', '#f4f0e6', 0.9)
glass = H.pbr('hang_glass', '#ffffff', 0.03, alpha=0.1, spec=0.9, blend='BLEND')
img = _photo.photo('picture_frame_hanging_01_art', 0, W=384, Hh=512, seed=11)
art = H.pbr('art_print', '#ffffff', 0.6, base_tex=img)
W, Hh, F, T, M = 0.5, 0.7, 0.04, 0.03, 0.06
...
H.finish('picture_frame_hanging_01', extras={'texres': 512})
