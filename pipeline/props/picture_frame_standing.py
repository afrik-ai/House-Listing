# Standing oak photo frame 20x25 with ivory mount and glass, dusk-hills print, easel back; front +Z.
import sys; sys.path.insert(0, __import__('os').path.dirname(__import__('os').path.abspath(__file__)))
import helpers as H, _kit as K, _photo
from helpers import V, PI
H.reset()
frame = K.wood('frame_oak', seed=19)
...
img = _photo.photo('picture_frame_standing_photo', 0, seed=2)
ph = H.pbr('photo', '#ffffff', 0.35, base_tex=img)
W, Hh, F, T = 0.2, 0.25, 0.022, 0.018
tilt = 0.18
parts = []
...
p = H.obj('photo', [(-W / 2 + F + 0.018, -0.0003, -Hh / 2 + F + 0.018), (W / 2 - F - 0.018, -0.0003, -Hh / 2 + F + 0.018), (W / 2 - F - 0.018, -0.0003, Hh / 2 - F - 0.018), (-W / 2 + F + 0.018, -0.0003, Hh / 2 - F - 0.018)],
...
H.finish('picture_frame_standing', extras={'texres': 512})
